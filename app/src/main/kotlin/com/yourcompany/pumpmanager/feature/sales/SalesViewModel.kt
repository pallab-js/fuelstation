...
class SalesViewModel @Inject constructor(
    ...
    fun onEvent(event: SalesEvent) {
        when (event) {
            is SalesEvent.FuelSelected -> {
                _state.update { 
                    it.copy(
                        selectedFuel = event.fuel,
                        pricePerLiter = event.fuel.price
                    ) 
                }
                recalculateTotal()
            }
            is SalesEvent.VolumeDigitEntered -> {
                _state.update { it.copy(volume = it.volume + event.digit) }
                recalculateTotal()
            }
            ...
            is SalesEvent.PaymentModeChanged -> {
                _state.update { it.copy(paymentMode = event.mode) }
            }
            SalesEvent.SaveSale -> {
                saveSale()
            }
            SalesEvent.DismissError -> {
    ...
    private fun recalculateTotal() {
        ...
        val total = vol * _state.value.pricePerLiter
        _state.update { it.copy(calculatedTotal = total) }
    }
    ...
    private fun saveSale() {
        ...
        if (vol <= 0.0) {
            _state.update { it.copy(errorMessage = "Please enter volume") }
            return
        }
        ...
        viewModelScope.launch {
            ...
            try {
                val sale = SaleEntity(
                    ...
                    shiftId = "current_shift", // TODO: Get from ShiftManager
